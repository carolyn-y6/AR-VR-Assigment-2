...
void GY_85::updateQuaternion()
{
    // Read sensor data
    int* acc_data = readFromAccelerometer();
    float* gyro_data = readGyro();
    // int* mag_data = readFromCompass();   // COMMENT OUT (no working mag)

    // Convert accelerometer data from raw LSB to g
    float ax = (float)accelerometer_x(acc_data) / 128.0f;
    float ay = (float)accelerometer_y(acc_data) / 128.0f;
    float az = (float)accelerometer_z(acc_data) / 128.0f;

    // Convert gyro data from deg/s to rad/s
    float gx = gyro_x(gyro_data) * DEG_TO_RAD;
    float gy = gyro_y(gyro_data) * DEG_TO_RAD;
    float gz = gyro_z(gyro_data) * DEG_TO_RAD;

    // Fake magnetometer values so the math doesn’t blow up
    // (Mahony implementation in your file *ignores* mx,my,mz anyway)
    float mx = 1.0f;
    float my = 0.0f;
    float mz = 0.0f;

    // Force Mahony filter (acc+gyro only)
    quat_filter.select_filter(QuatFilterSel::MAHONY);
    quat_filter.update(ax, ay, az, gx, gy, gz, mx, my, mz, q);

    // Convert quaternion to Euler angles
    quat_filter.quaternionToEuler(q, rpy);
}
